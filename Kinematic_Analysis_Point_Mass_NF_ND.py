...
samplerate = 100 #Set the sample rate for the specified device
...
for n in range(rows - 1):
    ...
    result_x = num_integration(time, velocity_x, position_x)
    result_y = num_integration(time, velocity_y, position_y)
    result_z = num_integration(time, velocity_z, position_z)
...
######################################################################
#Create .csv files containing data in x,y,z for velocity and position
kinematic_data = merge_list(time, acceleration_x, acceleration_y, acceleration_z, velocity_x,velocity_y, velocity_z, position_x, position_y, position_z)
np.savetxt("kinematic_data.csv", velocity_data, delimiter=',')

GraphingModule.graph(time, position_x, position_y, position_z, 'ye', 'ye', 1)
